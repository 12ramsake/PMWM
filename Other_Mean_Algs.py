...
# def output_perturbation(sigma,d):
#     ND=torch.distributions.Normal(torch.Tensor([0]), torch.Tensor([sigma]))
#     vec = ND.sample(torch.Size([d**2])).reshape(d,d)
#     # The function np.triu_indices returns the indices of the upper triangle of a square matrix, excluding the main diagonal.
#     iu = np.triu_indices(d,1)
#     il = (iu[1],iu[0])
#     vec[il]=vec[iu]
#     return vec
...
# # def torch_geomspace(start, end, steps, dtype=None, device=None):
# #     # Take the log of the start and end points
# #     log_start = torch.log(torch.tensor(start, dtype=dtype, device=device))
# #     log_end = torch.log(torch.tensor(end, dtype=dtype, device=device))
    
# #     # Create a linearly spaced tensor in the logarithmic domain
# #     result = torch.logspace(log_start, log_end, steps=steps, base=np.exp(1), dtype=dtype, device=device)
...
# # end = 1000
# # steps = 5
# # geomspace_tensor = torch_geomspace(start, end, steps)
# # print(geomspace_tensor)
# def get_bins(d,B):
#     maxx=int(2+np.log((B**2)*d))
#     b=torch.ones(maxx+2)
...
# def filter_1d(tail_indices, tau, epsilon, delta, B, d):
#     # print('B ', B)
#     # print('Tau' ,tau[tail_indices])
#     n = tau.shape[0]
#     # print('shape',tau.shape)
#     # print('tail_ind',tail_indices)
#     # print(torch.sum(tau[tail_indices]-1))
#     psi = torch.sum(tau[tail_indices]-1).item()/n+np.random.laplace(0,B**2*d/(n*epsilon),1)
#     # bins = torch_geomspace(1/4, B**2*d, 2+int(np.log(B**2*d)))
#     bins=get_bins(d,B)
#     # print("bins", bins)
#     hist,  _ = torch.histogram(tau[tail_indices], bins=bins)
#     # print("hist", hist)
#     hist = hist/n
#     # m = torch.distributions.Laplace(torch.tensor([0.0]), torch.tensor([1.0]))
#     # rdn=m.sample(hist[hist!=0].size() )
#     # print(hist[hist!=0].size())
#     # print(rdn.size())
#     # hist[hist!=0]=hist[hist!=0]+rdn
#     # hist[hist<2*np.log(1/delta)/(epsilon*n)+1/n]=0
#     # hist[hist!=0]=hist[hist!=0]+torch.random.laplace(0, 2/(n*epsilon),size=len(hist[hist!=0]))
#     for h in range(len(hist)):
#         if hist[h]!=0:
#             hist[h] += np.random.laplace(0, 2/(n*epsilon),1)
#         if hist[h]<2*np.log(1/delta)/(epsilon*n)+1/n:
#             hist[h] = 0
#     # print(psi[0])
#     for l in range(len(bins)-2, -1,-1):    
#         # print('LHS' ,torch.sum((bins[(l+1):]-bins[l])*hist[l:]))
#         if torch.sum((bins[(l+1):]-bins[l])*hist[l:])>=0.3*psi[0]:
#             return bins[l]
#     # return 
#     # print("Did not find rho ", hist)
#     return None
...
#     x_bar = torch.zeros(d)
#     for i in range(d):
#         bins = torch.linspace(-R-0.5, R+0.5, 2*int(R))
#         hist,  _ = torch.histogram(X[:,i], bins=bins)
#         hist = hist/n
#         for h in range(len(hist)):
#             if hist[h]!=0:
#                 hist[h] += np.random.laplace(0, 2/(n*epsilon/d))
#             if hist[h]<2*np.log(d/delta)/(epsilon*n/d)+1/n:
#                 hist[h] = 0
#         x_bar[i] = bins[torch.argmax(hist)]+bins[torch.argmax(hist)+1]               
#     return x_bar, 4*np.sqrt(np.log(d*n/0.9))
...
#     for _ in range(T1):
#         lamb_s = torch.linalg.norm(M(X[S], n)-torch.eye(d), 2)+np.random.laplace(0,2*B**2*d/(n*epsilon_1))
        
#         if len(S)<0.55*n:
#             print('failed')
#             return None
#         SN=torch.distributions.Normal(0,2*B*np.sqrt(2*d*np.log(1.25/delta_1))/(n*epsilon_1))
#         output = torch.mean(X[S], axis=0)+SN.sample( (d,) )
#         if lamb_s<C*alpha*np.log(1/alpha):
#             # print('succeded, '+str(S.max())+str("  ")+str(torch.linalg.norm(output))+'  '+str(torch.sum(S>=n-int(alpha*n))))
#             return output
#         # else:
#             # print('l',lamb_s)
#             # print(C*alpha*np.log(1/alpha))
#         alpha_s = 1/(10*(0.01/C+1.01)*lamb_s)

#         Sigma_list = []
#         for _ in range(T2):
#             lamb_t = torch.linalg.norm(M(X[S], n)-torch.eye(d), 2)+np.random.laplace(0,2*B**2*d/(n*epsilon_2))
#             if lamb_t<lamb_s*0.5:
#                 # print('next epoch')
#                 break
#             else:
...
#                 Sigma_list.append(Sigma)
#                 # print(Sigma_list)
#                 # print(torch.stack(Sigma_list).shape )
#                 sum_sigma = (alpha_s.item())*(torch.stack(Sigma_list)-torch.eye(d)).sum(0)
#                 U = torch.exp(sum_sigma)/torch.trace(torch.exp(sum_sigma))

#                 psi = torch.trace((M(X[S], n)-torch.eye(d))@(U.T))+np.random.laplace(0,2*B**2*d/(n*epsilon_2))
#                 if psi <= lamb_t/5.5:
#                     continue
#                 else:
#                     SN=torch.distributions.Normal(0,2*B*np.sqrt(2*d*np.log(1/delta_2))/(n*epsilon_2))
#                     mu_t = torch.mean(X[S], axis=0)+SN.sample( (d,) )
#                     tau = (((X-mu_t)@U)*(X-mu_t)).sum(1)
#                     strd=tau[S]
#                     torch.sort(strd,0)
#                     # print(tau[S].size())
#                     # print(strd.size())
#                     # print(len(S)-2*int(alpha*n))
#                     sorted_tau_thres = strd[len(S)-2*int(alpha*n)]
                    
#                     tail_indices = []
#                     for l in S:
#                         if tau[l]>=sorted_tau_thres:
#                             tail_indices.append(l.item())
#                     rho = filter_1d(tail_indices, tau, epsilon=epsilon_2, delta=delta_2, B=B, d=d)
#                     print(rho)
#                     S_remove = []
#                     good = []
...
#                     for ind in tail_indices:
#                         # print(tau[ind].item)
#                         # print(torch.rand(1).item()*rho)
#                         if tau[ind].item()>= (torch.rand(1).item()*rho):
#                             if ind>=n-int(alpha*n):
#                                 bad.append(ind)
#                             else:
#                                 good.append(ind)
...
#                     # print('S',S)
#                     # print('Sbad',S_remove)
#                     mask = ~torch.isin(S, torch.Tensor(S_remove))
#                     S = S[mask]
#                     # S = torch.setdiff1d(S, torch.Tensor(S_remove))
#     return output
...
#     for i in range(d):
#         X[:,i] = torch.clip(X[:,i], (x_bar[i]-B).item(),  (x_bar[i]+B).item()) 
...
#                 S_bad.append(i)
#     S = S[~torch.isin(S, torch.Tesnor(S_bad))]
#     return torch.mean(X[S], axis=0)
...
# PRIME(epsilon=10, delta=0.01, X=X, alpha=0.05, R=10)
...
# def DPMean_mv(data,alpha,R,eps,beta=0.1):
#     m=int(5*np.log(2/beta))
#     # n=len(data)
#     n, d=data.shape
#     j=int(n/2)
...
#         # Private DP histogram
#         for j in range(d):
#             counts, bin_edges = dpl.tools.histogram(Zi[:,j], eps/(2*d),
#                                                 bins=bins, range=(-R - 2*r,R + 2*r))
#             index_of_max = np.argmax(counts)
#             interval=np.array([bin_edges[index_of_max]-2*r,2*r+bin_edges[index_of_max+1]])
#             # interval=PDPRE(Zi,alpha,R,eps/2)
...
#             Yi=np.clip(Yi1,interval[0],interval[1])
#             # print(np.mean(Yi))
#             means[i,j]=np.mean(Yi)+np.random.laplace(2*d/eps,size=1)*ri/k
...
#     bins = np.arange(-R - 2 * r, R + 2 * r + 1, step_size)
#     counts, bin_edges = dpl.tools.histogram(data, eps, bins=bins, range=(-R - 2 * r, R + 2 * r))
#     index_of_max = np.argmax(counts)
#     # Compute interval around max bin
#     I = np.array([bin_edges[index_of_max], bin_edges[index_of_max + 1]])
#     return I
...
#     j = n // 2
#     k = j // m
#     # Split data into two parts Z and W
#     Z = data[:j]
#     W = data[j:]
...
#     for i in range(m):
#         # Batch slice the data for Z and W
#         Zi = Z[i * k:(i + 1) * k]
#         interval = PDPRE(Zi, alpha, R, eps_2)
#         ri = interval[1] - interval[0]
        
#         # Clip W data and compute the mean for Yi
#         Yi = np.clip(W[i * k:(i + 1) * k], interval[0], interval[1])
#         muhat_i = np.mean(Yi) + np.random.laplace(scale=laplace_scale) * (ri * m / j)
        
#         # Assign to the means array
#         means[i] = muhat_i
...
# def order_stat(data,k):
#     return np.partition(data, k-1)[k-1]
